...
- [x] **Stale-turn recovery FIXED (the "stuck on ORB_LOADING forever" hang)** — when the server misses end-of-turn (gets the full turn but only sends `ping`, never a response), a two-stage `heartbeat_task` watchdog off `s_ptt_close_us`: **stage 1 (8 s)** re-sends the tail pad (routed through `mic_task` via `s_pad_resend_request`) to nudge the server VAD; **stage 2 (16 s)** reverts UI to `ORB_MUTED` so the orb is usable. NFC injects get the same recovery for free. Details in [CLAUDE.md](CLAUDE.md) "Stale-turn recovery" + [CONVAI.md](CONVAI.md).
