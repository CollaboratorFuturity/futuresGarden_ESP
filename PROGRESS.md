...
- [x] **PCM ring** (**512 KB** PSRAM `xStreamBufferCreateStatic`, was 128 KB) — buffer between WS receive and speaker, sized to hold ~a whole turn so the WS task never blocks mid-turn (see memory-cascade entry below)
...
- [ ] **Latent: PTT-during-inject-pad buffer race** — the custom-phrase turn-end pad runs `send_silence_frames()` from `nfc_task`, touching the `mic_task`-owned `s_mic_pcm`/`s_mic_b64`/`s_mic_json`. If PTT is pressed during the pad (UI is `ORB_LOADING`; `mic_task`'s rising edge only blocks on `s_agent_speaking`), both tasks write those buffers. Not observed as a fault; the §6.4 gate makes it hard to hit. Fix: route the pad through `mic_task` via a flag, per BEHAVIOR.md §3.5. See `convai.c::convai_send_user_message`.
- [x] **Post-inject WS disconnect FIXED** — the inject pad now aborts the instant agent audio arrives (`s_turn_response_started` + `send_silence_frames(n, abortable=true)`), so it no longer contends with the WS-lib recursive lock that `handle_audio_event` holds while draining audio. (The earlier "pong not skipped during inject" theory was **wrong** — pong runs on the lock-holding WS task, so it never contends. Confirmed by reading `esp_websocket_client.c`.) Reconnect delay also cut 5000→1500 ms for faster recovery from genuine drops.
- [x] **Internal-SRAM exhaustion stabilized (crash + disconnect + `wifi:m f null` cascade)** — internal SRAM (~13 KB free, cratered to 0 during long audio turns) was the root of the `convai_send` stack overflow (→ hard reset), the TCP-send failure (→ WS disconnect), and WiFi frame drops. Fixes, all "keep big/frequent allocs in PSRAM": convai stacks → PSRAM + `sender`/`heartbeat` 4/3 KB→8 KB; **WiFi/lwIP → PSRAM** (`CONFIG_SPIRAM_TRY_ALLOCATE_WIFI_LWIP`) + `DYNAMIC_RX_BUFFER_NUM` 32→64; **cJSON → PSRAM** (`main.c cJSON_InitHooks`); **PCM ring** 128 KB→512 KB. Verified stable over 10 back-to-back injects / 160 s. Full write-up in [CLAUDE.md](CLAUDE.md) "Memory" + [CONVAI.md](CONVAI.md) "Audio receive back-pressure". Config knobs are now in `sdkconfig.defaults` (ship); coredump-to-flash stays bench-only (local `sdkconfig` + `partitions.coredump.csv`).
- [x] **Stale-turn recovery FIXED (the "stuck on ORB_LOADING forever" hang)** — when the server misses end-of-turn (gets the full turn but only sends `ping`, never a response), a two-stage `heartbeat_task` watchdog off `s_ptt_close_us`: **stage 1 (8 s)** re-sends the tail pad (routed through `mic_task` via `s_pad_resend_request`) to nudge the server VAD; **stage 2 (16 s)** shows `ORB_RETRY` ("No response / Please repeat", idle-equivalent — PTT + NFC work, persists until the user retries) plus a double 350 Hz beep so the drop is noticed. NFC injects get the same recovery for free. Details in [CLAUDE.md](CLAUDE.md) "Stale-turn recovery" + [CONVAI.md](CONVAI.md).
- [x] **`playback_task` moved to PSRAM + fail-hard start** — it was the last convai task on internal SRAM, spawned on the "one 4 KB internal stack still fits" assumption. False on release builds (OTA GitHub handshake leaves `heap_int` ~7 KB lower than a dev build): it intermittently failed to spawn → **no agent audio + UI wedged on `ORB_LOADING`**. Now `xTaskCreatePinnedToCoreWithCaps(MALLOC_CAP_SPIRAM)` like the other three; if it still can't spawn, `convai_start` tears down and returns `false`, and both callers retry 3× then drop to a non-`LOADING` state. All four convai stacks are now PSRAM (only the WS lib task stays internal). Details in [CLAUDE.md](CLAUDE.md).
- [x] **WiFi power-save disabled (`WIFI_PS_NONE`)** — fixed idle-only "random restarts": the default `WIFI_PS_MIN_MODEM` let the radio nap between beacons during idle (WSS up, only ping/pong traffic), and some APs deauthed the napping STA → new DHCP lease + WS reconnect + agent re-greet (uptime never reset, so not a crash). Set after `esp_wifi_start()` in `WIFI_Init`. Costs ~20–40 mA idle. Does **not** stop re-greets from genuine drops (that's `conversation_id` resume). Details in [CLAUDE.md](CLAUDE.md) WiFi section.
- [x] **First-turn disconnect storm FIXED (`SEND_TEXT_TIMEOUT_MS` 500→5000)** — on the first user turn (fresh TCP conn in slow-start, right after the ~200-330 KB greeting download), the uplink couldn't absorb the 43 KB/s mic burst within the 500 ms send timeout → `transport_poll_write` returned 0 (a timeout, not a real error) → WS lib disconnected + re-greeted. Hit ~65% of boots (up to 5 reconnects before a turn stuck). The send queue does NOT prevent it (disconnect is *inside* the send call). Raising the timeout lets `sender_task` ride out the transient congestion; the 1 MB queue absorbs the backlog (`q_depth` seen to 53 KB then drain, no drops). **Verified: 0 first-turn disconnects across repeated cold boots after the change.** Details in [CLAUDE.md](CLAUDE.md) send-queue section + [CONVAI.md](CONVAI.md).
- [ ] **Faster stale-turn detection via `user_transcript` — DATA VERIFIED, ready to implement.** The 16 s give-up is slow because it waits for a full agent response. On a healthy turn the server sends `user_transcript` (ASR of the user's audio, already parsed but only logged) *before* the agent replies; its **absence** is a much tighter "end-of-turn missed" signal. **Bench-confirmed over the silent + normal-turn test batches:** `user_transcript` is ALWAYS the first server event on a received turn (never `audio` first), lands at **+965–3447 ms** (the 3447 ms tail was a 5 s turn whose send queue backed up to 53 KB — see the anchor note below), and is **entirely absent** on all 8 missed (silent) turns. No cold-start penalty (it's ASR, not the LLM), so **no adaptive first-turn logic is needed** — the earlier "adaptive timeout" TODO is moot. **Plan:** key the two-stage watchdog on `user_transcript` absence — nudge at **~5 s**, give up at **~10 s** — and cancel the missed-turn watchdog the instant `user_transcript` arrives (then rely on the existing 10 s playback safety for a mid-turn vanish). 10 s give-up sits ~3× over the worst observed 3.4 s, so a slow-but-valid long turn is never aborted.
  - **Refinement (do only if a long congested turn ever creeps toward the give-up threshold): anchor the recovery clock to "turn fully delivered," not PTT close.** The real driver of transcript latency is send **backlog** (`q_depth`), NOT raw turn length — a long turn on a healthy link (`q_depth=0`) transcribes as fast as a short one; only the 53 KB-backlog turn was slow. So instead of padding the timeout by `real_audio_ms` (a weak proxy), start the stale countdown when `q_depth` drains back to 0 after the tail pad — i.e. when the server actually *has* the full turn. That self-adjusts for long/congested turns with no length lookup table. Not needed for anything observed so far (fixed 5 s/10 s covers it); keep in pocket.
- [ ] **Deferred: decode-decouple refactor** — residual `wifi:m f null` on *very* long single turns is the base64 decode being CPU-bound on the WS task. Full cure = move decode + PCM-push to a new `decode_task`. ~1–2 days, HIGH regression risk (interruption reset-vs-send race; end-of-turn ordering clips last word). Not needed to ship; do post-launch only if jitter bugs users. Analysis in [CONVAI.md](CONVAI.md) "decode-decouple refactor".
