...
void ST7701S_screen_init(ST7701S_handle St7701S_handle, unsigned char type)
{
    ...
    SPI_WriteComm(0x11);       // Sleep Out — starts the VGH/VGL bias charge pump
    Delay(200);                // ms — pump stabilize. Datasheet min is 120; bumped
                               // to give the pump margin to reach regulation at the
                               // lower input voltage seen on battery (vs USB).
...
// Cold-boot rail settle before the panel power-on sequence. On USB the 3.3V
// rail is rock-solid by the time this runs; on battery the cold-boot inrush
// leaves it still climbing, and the ST7701S bias charge pump (VGH/VGL ~±15V
// generated on-module from 3.3V) can't reach regulation against a low/moving
// input — the panel then latches blank even though digital init "succeeds"
// and the backlight (independent LEDC PWM on GPIO6) lights up. Waiting here
// gives the rail time to reach full voltage first. Pure boot latency; no
// effect on steady-state refresh, so it cannot reintroduce tearing.
#define LCD_RAIL_SETTLE_MS 250

void LCD_Init(void)
{
    /********************* LCD *********************/
    vTaskDelay(pdMS_TO_TICKS(LCD_RAIL_SETTLE_MS));
    ST7701S_reset();
