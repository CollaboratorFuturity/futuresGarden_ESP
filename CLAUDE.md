...
- The `heartbeat_task` stale-turn watchdog is now **two-stage**, both one-shot, both keyed off `s_ptt_close_us` (cleared by `process_message` on any non-ping/pong event, which cancels whichever stage hasn't fired):
  ...
  - **Stage 2 — give up (`PTT_STALE_TURN_RECOVER_MS` = 16000):** show `ORB_RETRY` ("No response / Please repeat", warm amber `0xCB6D2E`) + clear the watchdog so the orb is usable again instead of frozen. 16 s is comfortably past the ~10 s cold-start max, so a slow-but-valid turn is never aborted. Safe if a late response arrives — its audio flips the UI to `ORB_AGENT`. `ORB_RETRY` is an **idle-equivalent** state: PTT works (never UI-gated) and NFC polling is enabled for it (`orb_ui.c` gates `NFC_Set_Polling` on `ORB_MUTED || ORB_RETRY`), so the prompt persists until the user presses to retry — a press flips to `ORB_USER_TALK` normally. Don't confuse `ORB_RETRY` (recoverable, calm) with the red `ORB_ERROR` ("Restart the Orb", fatal). Stage 2 also plays a **double 350 Hz beep** (`i2s_audio_beep` ×2 with a 70 ms gap — the NFC blopp's top tone) so a user not looking at the orb still notices the drop; it's safe from `heartbeat_task` because the PCM ring is empty at give-up (no playback contends) and the beep does no TLS/heap work.
...
- **Architecture** (all in `Convai/convai.c`):
  ...
  - **`SEND_TEXT_TIMEOUT_MS` is 5000 (was 500) — load-bearing for first-turn stability.** The timeout passed to `esp_websocket_client_send_text` propagates to the transport's `poll_write`. At 500 ms it was a hair-trigger: on the **first user turn** (right after the ~200-330 KB greeting download, on a fresh TCP conn still in slow-start) the uplink can't absorb the 43 KB/s mic burst within 500 ms → `transport_poll_write` returns 0 (a *timeout*, not a real error — `transport_error=ESP_OK`, `esp_tls_err`/`sock_errno` are uninitialized garbage) → the WS lib treats the zero-length write as a dead transport and **disconnects + re-greets**. Observed on ~65% of boots (sometimes 5 reconnects before a turn stuck); the tell is `ws error … tx_age≈500ms` immediately followed by `Reconnect after 1500 ms`. **The send queue does NOT prevent this** — the disconnect happens *inside* the send call, before the retry logic runs — so this is a separate fix from the queue itself. Raising the timeout lets `sender_task` ride out the transient congestion; the 1 MB queue absorbs the mic backlog while a send blocks (`q_depth` grows — observed to 53 KB on a 5 s first turn — then drains, `q_block=0 q_retry=0`, no frames dropped). Kept under `WS_NETWORK_TIMEOUT_MS` (10 s) so a genuinely dead transport is still caught by the lib. Verified on hardware: 0 first-turn disconnects across repeated cold boots after the change. Don't drop it back toward 500.
...
- **WiFi power-save is DISABLED** — `esp_wifi_set_ps(WIFI_PS_NONE)` right after `esp_wifi_start()` in `WIFI_Init`. Default is `WIFI_PS_MIN_MODEM` (radio naps between DTIM beacons when traffic is light). The orb holds a persistent WSS across idle stretches where the only traffic is the ~2 s ping/pong — light enough that the radio sleeps most of the time, and some APs then deauth the napping-but-still-associated STA. Symptom was a "random restart" while **idle**: uptime never reset (not a crash), a new DHCP lease appeared, WS reconnected as `conn#N+1`, and the agent re-greeted. `WIFI_PS_NONE` keeps the radio awake so the AP never has a napping client to misjudge. Costs ~20–40 mA extra idle draw — fine on USB, acceptable on battery for the link stability. Must be set **after** `esp_wifi_start()`. Don't re-enable power save to save current without accepting the idle-drop/re-greet regression. (Note: this does NOT stop re-greets from *genuine* drops — that needs `conversation_id` resume, see PROGRESS.md.)
